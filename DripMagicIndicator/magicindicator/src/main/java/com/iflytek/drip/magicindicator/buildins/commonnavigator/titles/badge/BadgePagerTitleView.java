package com.iflytek.drip.magicindicator.buildins.commonnavigator.titles.badge;
...
import com.iflytek.drip.magicindicator.buildins.commonnavigator.IMeasurablePagerTitleView;
...
public class BadgePagerTitleView extends FrameLayout implements IMeasurablePagerTitleView {
    ...
    public void setXBadgeRule(BadgeAnchor badgeAnchor,int dp){
        setXBadgeRule(setBadgeRule(badgeAnchor,dp));
    }

    private  BadgeRule setBadgeRule(BadgeAnchor badgeAnchor,int dp) {
        int offset = 0;
        if (dp <= 0)
            offset = -UIUtil.dip2px(context, -dp);
        else
            offset = UIUtil.dip2px(context, dp);
        BadgeRule badgeRule = new BadgeRule(badgeAnchor, offset);
        return badgeRule;
    }
    ...
    public void setYBadgeRule(BadgeAnchor badgeAnchor,int dp){
        setYBadgeRule(setBadgeRule(badgeAnchor,dp));
    }
